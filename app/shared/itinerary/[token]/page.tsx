...
import { ArrowLeft, Calendar, Loader2, SearchX, User } from "lucide-react";
...
export default function SharedItineraryPage() {
  ...
  const primary = Array.isArray(snap?.primaryItinerary)
    ? snap.primaryItinerary
    : [];
      ...
      {loading ? (
        ...
        <div className="pt-20 p-6 lg:p-10 max-w-4xl mx-auto space-y-8">
          <div>
            ...
            {data.subtitle ? (
              <div className="mt-2 text-sm font-bold text-bb-text-muted flex items-center gap-2">
                <Calendar size={16} />
                {data.subtitle}
              </div>
            ) : null}
            <div className="mt-4 flex flex-wrap items-center gap-4 text-sm font-bold text-bb-text-muted">
              <span className="rounded-full border border-bb-border bg-bb-surface px-3 py-1 text-[10px] font-black uppercase tracking-wider text-bb-text-muted">
                Read-only
              </span>
            </div>
          </div>
          ...
          {isTripLayout && Array.isArray(primary) ? (
            <div className="bg-bb-surface rounded-[2.5rem] border border-bb-border shadow-sm p-8">
              <RainyDayToggle
                trip={{
          ...
          {!isTripLayout &&
          Array.isArray(groupEvents) &&
          groupEvents.length > 0 ? (
            <div className="space-y-4">
              <h2 className="text-2xl font-black text-bb-text tracking-tight px-1">
                Itinerary
              </h2>
              <ul className="space-y-3">
                {groupEvents.map((ev, i) => (
                  <li key={`${ev.title}-${i}`}>
                    <Card className="rounded-2xl border border-bb-border shadow-sm">
                      <CardContent className="p-6">
                        <p className="font-bold text-bb-text">{ev.title}</p>
          ...
          {!isTripLayout && (!groupEvents || groupEvents.length === 0) ? (
            <p className="text-bb-text-muted text-sm">
              This shared itinerary has no displayable events in the saved
              snapshot.
            </p>
          ) : null}
        </div>
        ...
        <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-6">
          <div className="bg-white border border-gray-200 rounded-[2rem] p-10 shadow-sm flex flex-col items-center gap-4">
            ...
            <Link href="/">
              <Button className="mt-4 rounded-xl text-bb-text-muted">
                {"<-- Go Back"}
              </Button>
            </Link>
          </div>
